...
class _DraggableTableState extends State<_DraggableTable> {
  ...
  void initState() {
    super.initState();
    _currentX = widget.table.posX ?? 0;
    _currentY = widget.table.posY ?? 0;
  }

  @override
  void didUpdateWidget(_DraggableTable oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Accept model updates when we're not actively dragging.
    // The drag flag is set on first pan and cleared on pan end.
    if (!_dragging) {
      _currentX = widget.table.posX ?? 0;
      _currentY = widget.table.posY ?? 0;
    }
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => widget.onTableTap?.call(widget.table),
      onPanStart: (_) => _dragging = true,
      onPanUpdate: (details) {
        _currentX = (_currentX + details.delta.dx).clamp(
          0,
          widget.canvasSize - widget.tableSize,
        );
        _currentY = (_currentY + details.delta.dy).clamp(
          0,
          widget.canvasSize - widget.tableSize,
        );
        setState(() {});
      ...
      onPanEnd: (_) {
        widget.onMoved?.call(Offset(_currentX, _currentY));
        _dragging = false;
      },
      child: Transform.translate(
        ...
        child: _TableWidget(table: widget.table, size: widget.tableSize),
      ),
    );
  }
}
