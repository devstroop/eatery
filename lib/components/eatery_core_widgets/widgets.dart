/// App-layer component library — migrated from packages/eatery_core.
///
/// Pure UI widgets that depend only on design tokens (theme/) and utilities.
library;

export 'app_adaptive_shell.dart';
export 'app_badge.dart';
export 'app_bottom_sheet_grip.dart';
export 'app_button.dart';
export 'app_card.dart';
export 'app_category_chip.dart';
export 'app_dialog.dart';
export 'app_empty_state.dart';
export 'app_error_boundary.dart';
export 'app_form_field.dart';
export 'app_menu_tile.dart';
export 'app_multistep_form.dart';
export 'app_notification.dart';
export 'app_notification_banner.dart';
export 'app_order_card.dart';
export 'app_page_shell.dart';
export 'app_product_card.dart';
export 'app_search_field.dart';
export 'app_select_card.dart';
export 'app_skeleton.dart';
export 'app_status_timeline.dart';
export 'app_table_view.dart';
export 'app_text_field.dart';
