...
class _AddReservationPageState extends ConsumerState<AddReservationPage> {
  ...
  @override void initState() { super.initState();
    if (widget.reservation != null) {
      ...
      _status = widget.reservation!.status; _tableId = widget.reservation!.diningTableId;
      _noteCtrl.text = widget.reservation!.note ?? "";
    }
  }
  @override void dispose() { _nameCtrl.dispose(); _phoneCtrl.dispose(); _noteCtrl.dispose(); super.dispose(); }

  Future<void> _save() async {
    if (!_formKey.currentState!.validate()) return;
    final repo = ReservationRepository(ref.read(eateryStoreProvider));
    await repo.saveReservation(Reservation(
      customerName: _nameCtrl.text.trim(), customerPhone: _phoneCtrl.text.trim().isEmpty ? null : _phoneCtrl.text.trim(),
      diningTableId: _tableId, partySize: _partySize, dateTime: _dateTime, duration: _duration,
      status: _status, note: _noteCtrl.text.trim().isEmpty ? null : _noteCtrl.text.trim(),
  ...
  Widget build(BuildContext context) {
    final tables = SqliteDiningTableRepository(store: ref.read(eateryStoreProvider)).getAllTables();
    return AppPageShell(
      title: widget.reservation != null ? "Edit Reservation" : "Add Reservation",
      color: AppColors.primary,
      actions: [IconButton(icon: const Icon(Icons.save), onPressed: _save)],
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Form(key: _formKey, child: ListView(children: [
          TextFormField(controller: _nameCtrl, decoration: const InputDecoration(labelText: "Customer Name", border: OutlineInputBorder()),
            validator: (v) => v == null || v.trim().isEmpty ? "Required" : null),
          const SizedBox(height: 12),
          TextFormField(controller: _phoneCtrl, decoration: const InputDecoration(labelText: "Phone (optional)", border: OutlineInputBorder()),
          ...
          ListTile(
            ...
            onTap: () async {
              final dt = await showDatePicker(context: context, firstDate: DateTime.now(), lastDate: DateTime.now().add(const Duration(days: 90)), initialDate: _dateTime);
              if (dt == null) return;
              final tm = await showTimePicker(context: context, initialTime: TimeOfDay.fromDateTime(_dateTime));
              if (tm != null) setState(() => _dateTime = DateTime(dt.year, dt.month, dt.day, tm.hour, tm.minute));
            },
          ),
          const SizedBox(height: 12),
          DropdownButtonFormField<int>(
            decoration: const InputDecoration(labelText: "Table", border: OutlineInputBorder()),
            value: _tableId, items: [
              const DropdownMenuItem(value: null, child: Text("Auto-assign")),
              ...tables.where((t) => t.status == DiningTableStatus.available || t.id == _tableId).map((t) =>
                DropdownMenuItem(value: t.id, child: Text("${t.name} (Cap: ${t.capacity})"))),
            ], onChanged: (v) => setState(() => _tableId = v),
          ),
          const SizedBox(height: 12),
          DropdownButtonFormField<int>(
            decoration: const InputDecoration(labelText: "Status", border: OutlineInputBorder()),
            value: _status, items: const [
          ...
          const SizedBox(height: 12),
          TextFormField(controller: _noteCtrl, decoration: const InputDecoration(labelText: "Notes", border: OutlineInputBorder()), maxLines: 2),
        ])),
      ),
    );
  }
