...
class _AddDiscountPageState extends ConsumerState<AddDiscountPage> {
  ...
  @override void dispose() { _nameCtrl.dispose(); _valueCtrl.dispose(); _minCtrl.dispose(); super.dispose(); }

  Future<void> _save() async {
    if (!_formKey.currentState!.validate()) return;
    final repo = DiscountRepository(ref.read(eateryStoreProvider));
    await repo.saveDiscount(Discount(
    ...
    return AppPageShell(
      ...
      child: Padding(
        ...
        child: Form(key: _formKey, child: ListView(children: [
          CustomTextFromField(controller: _nameCtrl, label: "Discount Name", hint: "e.g. Happy Hour 20%",
            validator: (v) => v == null || v.trim().isEmpty ? "Required" : null),
          AppSpacing.gapMd,
          DropdownButtonFormField<int>(
            decoration: const InputDecoration(labelText: "Type", border: OutlineInputBorder()),
            value: _type, items: const [
          ...
          AppSpacing.gapMd,
          CustomTextFromField(controller: _valueCtrl, label: "Value", hint: "10 = 10% or 10.0",
            keyboardType: TextInputType.number,
            validator: (v) => v == null || v.trim().isEmpty ? "Required" : null),
          AppSpacing.gapMd,
          CustomTextFromField(controller: _minCtrl, label: "Minimum Order", hint: "Optional",
            keyboardType: TextInputType.number),
          AppSpacing.gapMd,
