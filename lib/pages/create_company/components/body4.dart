...
class Body4 extends StatelessWidget {
  ...
  Body4({Key? key, required this.themeColor, required this.edition, required this.taxNoController, required this.foodLicNoController, required this.defaultTaxController, required this.onTaxTypeChanged, required this.taxType}) : super(key: key);
      ...
      child: ListView(
        ...
        children: [
          ...
          CustomTextFromField(
            ...
            validator: (value) {
              if (value!.trim().isNotEmpty && !value.trim().isValidGSTIN()) return '${edition.name} license number is not valid';
              return null;
            },
          ),
          SpacingStyle.defaultVerticalSpacing,
          CustomTextFromField(
            ...
            hint: 'Enter ${edition == Edition.gst ? 'FSSAI' : 'Food'} license number...',
            focusNode: focus2,
            textInputAction: TextInputAction.next,
            onFieldSubmitted: (v) {
            ...
            validator: (value) {
              if (value!.trim().isNotEmpty && (value.trim().length < 10 || !value.trim().isNumericOnly)) return '${edition == Edition.gst ? 'FSSAI' : 'Food'} license number is not valid';
            ...
            children: [
              Flexible(
                child: CustomTextFromField(
                  ...
                  validator: (value) {
                    if (value!.trim().isNotEmpty && !value.trim().isNum) return 'Default ${edition.name} license number is not valid';
                    // if (edition == Edition.gst && !value!.trim().isValidGSTIN()) return '${edition.name} license number is not valid';
                    return null;
                  },
                  onFieldSubmitted: (v) {
...
class BAP4 extends StatelessWidget {
  ...
  final int? index;
  const BAP4({Key? key, required this.themeColor, this.callback, this.index}) : super(key: key);
