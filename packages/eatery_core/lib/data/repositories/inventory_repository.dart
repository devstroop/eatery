...
class InventoryRepository {
  ...
  List<Supplier> getAllSuppliers() => _store
  ...
  List<PurchaseOrderItem> getPurchaseOrderItems(int poId) => _store
      .query('SELECT * FROM purchase_order_item WHERE purchaseOrderId = ?', [
        poId,
      ])
      .map(PurchaseOrderItem.fromMap)
      .toList();
  ...
  List<StockAdjustment> getStockAdjustments(int productId) => _store
      .query(
        'SELECT * FROM stock_adjustment WHERE productId = ? ORDER BY createdAt',
        [productId],
      )
      .map(StockAdjustment.fromMap)
      .toList();
