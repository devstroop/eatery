...
class LoyaltyRepository {
  ...
  LoyaltyRepository(this._store);

  CustomerLoyalty? getByCustomer(int customerId) {
    final rows = _store.query(
      'SELECT * FROM customer_loyalty WHERE customerId = ? LIMIT 1',
      [customerId],
    );
  ...
  void recordVisit(int customerId, double spent) {
    ...
    if (existing != null) {
      ...
      _store.execute(
        'UPDATE customer_loyalty SET points = ?, totalVisits = totalVisits + 1, totalSpent = totalSpent + ?, lastVisitAt = ?, updatedAt = ? WHERE id = ?',
        [pts, spent, now, now, existing.id],
      );
      _store.execute(
        'INSERT INTO loyalty_transaction (customerId, points, type, referenceId, description, createdAt) VALUES (?,?,?,?,?,?)',
        [customerId, spent * 0.1, 0, null, 'Points earned from order', now],
      );
    } else {
      final pts = spent * 0.1;
      _store.execute(
        'INSERT INTO customer_loyalty (customerId, points, totalVisits, totalSpent, lastVisitAt, tier, createdAt, updatedAt) VALUES (?,?,1,?,?,0,?,?)',
  ...
  void redeemPoints(int customerId, double points, String description) {
    ...
    final now = DateTime.now().millisecondsSinceEpoch;
    _store.execute(
      'UPDATE customer_loyalty SET points = points - ?, updatedAt = ? WHERE id = ?',
      [points, now, loyalty.id],
    );
    _store.execute(
      'INSERT INTO loyalty_transaction (customerId, points, type, referenceId, description, createdAt) VALUES (?,?,?,?,?,?)',
