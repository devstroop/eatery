...
class SubscriptionRepository {
  ...
  Future<int> save(Subscription sub) async {
    ...
    if (sub.id != null) {
      _db.execute(
        'UPDATE subscription SET purchaseCode=?,validFrom=?,validTill=?,subscriptionType=? WHERE id=?',
        [...v, sub.id],
      );
      return sub.id!;
    }
    _db.execute(
      'INSERT INTO subscription (purchaseCode,validFrom,validTill,subscriptionType) VALUES (?,?,?,?)',
      v,
    );
  ...
  Subscription _toSub(Map<String, Object?> r) {
    ...
    return Subscription(
      ...
      validFrom: r['validFrom'] != null
          ? DateTime.fromMillisecondsSinceEpoch(r['validFrom'] as int)
          : null,
      validTill: r['validTill'] != null
          ? DateTime.fromMillisecondsSinceEpoch(r['validTill'] as int)
          : null,
      subscriptionType: st,
      id: r['id'] as int,
    );
