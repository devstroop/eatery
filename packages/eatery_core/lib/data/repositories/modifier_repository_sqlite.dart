...
class SqliteModifierRepository implements ModifierRepository {
  ...
  @override
  List<ModifierGroup> getAllGroups() => _store
      .query('SELECT * FROM modifier_group ORDER BY sortOrder')
      .map(ModifierGroup.fromMap)
      .toList();

  @override
  ModifierGroup? getGroupById(int id) {
    final rows = _store.query('SELECT * FROM modifier_group WHERE id = ?', [
      id,
    ]);
  ...
  @override
  List<Modifier> getModifiers(int groupId) => _store
      .query(
        'SELECT * FROM modifier WHERE modifierGroupId = ? ORDER BY sortOrder',
        [groupId],
      )
      .map(Modifier.fromMap)
      .toList();
  ...
  @override
  List<ModifierGroup> getGroupsForProduct(int productId) => _store
      .query(
  ...
  Future<void> setProductGroups(int productId, List<int> groupIds) async {
    _store.execute('DELETE FROM product_modifier WHERE productId = ?', [
      productId,
    ]);
