...
class _ModifierSheetState extends ConsumerState<ModifierSheet> {
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                ...
                children: widget.groups.map((group) {
                  final modifiers = ref.read(modifierRepositoryProvider).getModifiers(group.id!);
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Text(group.name, style: AppTypography.titleMedium.copyWith(fontWeight: FontWeight.bold)),
                              if (group.isRequired)
                                Container(
                          ...
                          if (group.description != null)
                            Text(group.description!, style: AppTypography.bodySmall),
                          const SizedBox(height: 8),
                          ...modifiers.map((m) {
                            final isSelected = selectedIds.contains(m.id);
                            return CheckboxListTile(
                              dense: true,
                              title: Text(m.name, style: AppTypography.bodyMedium),
                              subtitle: m.priceAdjust > 0
                                  ? Text('+${m.priceAdjust.toStringAsFixed(2)}',
                              ...
                              onChanged: (v) {
                                setState(() {
                                  if (v == true) {
                                    if (group.maxSelect > 0 && selectedIds.length >= group.maxSelect) return;
                                    _selected[group.id!] = [...selectedIds, m.id!];
                                  } else {
                                    if (selectedIds.length <= group.minSelect) return;
                                    _selected[group.id!] = selectedIds.where((id) => id != m.id).toList();
                                  }
                                  _recalcCost();
                                });
                              },
                          ...
                          if (modifiers.length > 1)
                            Text('${selectedIds.length}/${group.maxSelect > 0 ? group.maxSelect : "any"} selected',
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text('Add to Order  \$${(widget.product.salePrice ?? widget.product.mrpPrice) + _extraCost}'),
              ),
            ),
          ],
        ),
  ...
  void _recalcCost() {
    ...
    for (final entry in _selected.entries) {
      final modifiers = ref.read(modifierRepositoryProvider).getModifiers(entry.key);
