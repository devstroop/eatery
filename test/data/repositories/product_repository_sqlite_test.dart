...
/// End-to-end test of the SQLite-backed [SqliteProductRepository] against the
/// native libeaterystore, exercising enums, nullables, booleans and doubles.
void main() {
  ...
  test('filter by type and category', () async {
    ...
    await repo.saveProduct(
      makeProduct(name: 'Item', type: ProductType.inventoryItem, categoryId: 2),
    );

    expect(repo.getProductsByType(ProductType.kitchenDish).map((p) => p.name), [
      'Dish',
    ]);
    expect(
  ...
  test('search is case-insensitive and partial', () async {
    ...
    expect(repo.searchProducts('COFFEE').map((p) => p.name), ['Black Coffee']);
