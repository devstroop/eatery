...
/// End-to-end test of the SQLite-backed [SqliteCustomerRepository] against the
/// native libeaterystore. Exercises nullables, booleans (0/1) and DateTime
/// (epoch millis) round-trips.
///
/// `getOutstandingAmount` delegates to Hive-backed Orders/Payments (unchanged),
/// so it is not covered here; the customer CRUD methods never touch [_db].
void main() {
  ...
  setUp(() {
    ...
    // db is only used by getOutstandingAmount (not tested here); an
    // uninitialized instance is fine for the CRUD methods.
    repo = SqliteCustomerRepository(
      store: store,
      db: EateryDatabase(dataDir: '.'),
    );
  });
