...
>
> ⚠️ **Note:** This file replaces the original `ISSUES.md` tracker that listed ~106 codebase bugs/features. The original content has been archived to `ISSUES-ARCHIVE.md` (not created yet — do this before deleting the old tracker).
...
| S3 | Staff login → admin | Login as admin (StaffType.admin, PIN 1234) | Redirected to `/dashboard` with full admin menu |
| S4 | Staff login → waiter | Login as waiter (StaffType.waiter, PIN 1234). Requires issue 11a (waiter auth integration) to be completed first — the waiter sub-app currently has no auth flow. | Redirected to `/tables` with waiter view |
